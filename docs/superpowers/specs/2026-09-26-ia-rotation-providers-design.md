...
- **`key-pool.ts`** (nouveau) : `KeyPool` construit depuis un préfixe d'env (`KeyPool.fromEnv("GROQ_API_KEY")`), horloge injectable pour les tests. API : `available(): { index; key }[]` (clés non en pause ni désactivées, dans l'ordre), `pause(index, ms)`, `disable(index)`, `size` ; une clé présente deux fois dans l'env n'est gardée qu'une fois. Ne logge jamais une clé ; les logs utilisent l'index (`clé #2`).
