...
| Token | Hex | Usage |
|---|---|---|
| `emerald-500` | `#008558` | CTA principal, liens, focus ring, glow du hero 3D |
| `emerald-600` | `#00693F` | Hover/active des CTA |
| `emerald-300` | `#34D399` | Accent doux, halos, dégradés |

Vert plus saturé (S100%) qu'à l'origine (`#059669`), à luminosité légèrement réduite pour préserver le contraste du texte blanc sur les CTA (voir règle de contraste ci-dessous). Décision consignée dans `docs/DECISIONS.md`.
...
**Attention à ne pas confondre le vert de marque et le vert sémantique** : `emerald-500` (`#008558`, plus profond/teal) sert à la marque et aux CTA ; `success` (`#22C55E`, plus vif/jaune-vert) sert uniquement à indiquer un résultat financier positif. Les deux doivent rester visuellement distincts à l'écran (ne pas utiliser `success` comme couleur de bouton, ni `emerald-500` comme indicateur de résultat).
